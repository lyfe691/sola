 ...
 *
 * Server-side GitHub activity proxy. The token lives only here (process.env,
 * NOT a VITE_ var), so it never reaches the browser bundle. The client calls
 * /api/github-activity?username=... and gets the processed feed back.
 *
 * The ProcessedActivity shape below is the API contract and mirrors the type in
 * src/lib/github.ts (kept in sync deliberately to avoid a cross-tree import).
 */
...
interface GitHubEvent {
  ...
  payload: {
    ...
    pull_request?: { id: number; title: string; html_url: string; merged: boolean; additions: number; deletions: number };
  ...
  switch (event.type) {
    ...
    case "PullRequestEvent": {
      ...
      return {
        ...
        url: pr.html_url,
        metadata: { pullNumber: pr.id, additions: pr.additions, deletions: pr.deletions },
      };
    }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // cache at the edge so repeat visits don't re-hit GitHub
  res.setHeader(
    "Cache-Control",
