...
void main() {
  ...
  fragColor = vec4(auroraColor * auroraAlpha, auroraAlpha * uIntensity);
}
`;
...
export default function Aurora(props: AuroraProps) {
  ...
  useEffect(() => {
    ...
    // Get initial intensity based on theme
    const getThemeIntensity = () => {
      const lightMode = isLightTheme();
      return lightMode ? 0.45 : 0.8; // More visible in light mode, full effect in dark
    };
    ...
    program = new Program(gl, {
      ...
      uniforms: {
        ...
        uBlend: { value: blend },
        uIntensity: { value: getThemeIntensity() },
      },
    });
    ...
    const observer = new MutationObserver(() => {
      if (program) {
        program.uniforms.uIntensity.value = getThemeIntensity();
      }
    });
