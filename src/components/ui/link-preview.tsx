...
export const LinkPreview: React.FC<LinkPreviewProps> = ({ href, children, className, screenshot = true, previewType = "auto", compact = true }) => {
  const hostname = getHostname(href)
  const favicon = hostname ? `https://icons.duckduckgo.com/ip3/${hostname}.ico` : undefined
  const screenshotUrl = screenshot ? `https://image.thum.io/get/width/600/crop/600/noanimate/${encodeURIComponent(href)}` : undefined
  const [mode, setMode] = React.useState<"iframe" | "image">(
    previewType === "iframe" ? "iframe" : previewType === "image" ? "image" : "iframe"
  )
  ...
  return (
    <HoverCard>
      ...
      <HoverCardContent className="w-auto p-0 overflow-hidden bg-background/80 backdrop-blur-sm">
        <div className="relative">
          ...
          <div className="px-2 py-1 flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              <div className="text-xs text-muted-foreground truncate">{hostname || href}</div>
            </div>
          </div>
        </div>
      </HoverCardContent>
