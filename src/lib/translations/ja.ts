  ...
  projects: {
    ...
    list: {
      ...
      ura: {
        ...
        description:
          "Ura はフィンランド・ラップランド向けのスノーモービル用ルートナビです。iOS と Android のアプリと、データを整備する Web コンソールの二本立てで、公開されるバンドルは 2595 本のルートと周辺 3388 か所を端末に保存します。オンルート判定エンジンは 40〜60 m のヒステリシス帯で判定を保持し、分岐点でも表示がちらつきません。すべてのルートに、自由に走れるか、先に許可が必要かが示されます。",
      },
      montu: {
        title: "Montu",
      ...
      sigrid: {
        ...
        description:
          "母が焼くお菓子を紹介する、スペイン語のカタログサイトです。17 品を 4 つのカテゴリーで絞り込めて、問い合わせページは注文を受けずに Instagram と WhatsApp へ送り出します。母はもともとメッセージで注文を受けているので、サイトは会話が始まる手前で終わります。カタログは CMS ではなく TypeScript ファイル 1 つに入っていて、運用費はかかりません。",
      },
      qr: {
        title: "QR",
      ...
      vault: {
        ...
        description:
          "Vault はローカルの OIDC サンドボックスです。Keycloak 26 がトークンを発行し、背後に PostgreSQL、FastAPI がレルムの JWKS で検証し、Next.js のフロントエンドがロールで制限された admin と user のダッシュボードを表示します。レルム・クライアント・ロール・デモユーザーは terraform/main.tf で宣言されているので、ID 設定は管理コンソールの状態ではなくファイルです。docker compose のコマンド 1 つで立ち上がります。",
      },
      fleetmap: {
        title: "fleetmap",
        tagline: "配送車のGPSを事務所のモニターへ",
        description:
          "fleetmap は配送車両向けの監視コンソールです。各車両のスマートフォンが位置を送信し、オフィスの画面に全車両の動きが停車地・ETA・遅延とともに表示されます。リアルタイム配信は Supabase Realtime に任せ、Redis も自前の WebSocket サーバーもありません。経路はスイスの地図データで自己ホストした OSRM から取得します。壁掛けのタッチスクリーンで本番稼働中です。",
      },
    },
  },
  skills: {
