  ...
  projects: {
    ...
    list: {
      ...
      ura: {
        ...
        description:
          "Ura는 핀란드 라플란드를 위한 스노모빌 경로 내비게이션입니다. iOS·Android 앱과 데이터를 관리하는 웹 콘솔로 구성되며, 게시된 번들마다 2,595개의 경로와 주변 3,388곳을 휴대폰에 담습니다. 온루트 엔진은 40–60 m 히스테리시스 구간으로 판정을 유지해 갈림길에서도 깜빡이지 않습니다. 모든 경로에 자유 주행 가능 여부와 허가 필요 여부가 표시됩니다.",
      },
      montu: {
        title: "Montu",
      ...
      sigrid: {
        ...
        description:
          "홈베이커인 어머니를 위한 스페인어 카탈로그 사이트입니다. 17가지 작품을 네 개의 카테고리 필터로 볼 수 있고, 연락 페이지는 주문을 받는 대신 Instagram과 WhatsApp으로 연결합니다. 어머니는 메시지로 판매하시기 때문에 사이트는 대화가 시작되는 지점에서 멈춥니다. 카탈로그는 CMS 대신 TypeScript 파일 하나에 들어 있어 운영 비용이 들지 않습니다.",
      },
      qr: {
        title: "QR",
      ...
      vault: {
        ...
        description:
          "Vault는 로컬 OIDC 샌드박스입니다. Keycloak 26이 토큰을 발급하고, 뒤에는 PostgreSQL이 있으며, FastAPI가 realm의 JWKS로 토큰을 검증하고, Next.js 프런트엔드가 역할별로 제한된 admin과 user 대시보드를 보여줍니다. realm, 클라이언트, 역할, 데모 사용자는 terraform/main.tf에 선언되어 있어 ID 설정이 관리 콘솔의 상태가 아니라 파일입니다. docker compose 명령 하나로 실행됩니다.",
      },
      fleetmap: {
        title: "fleetmap",
        tagline: "차량 GPS를 사무실 화면으로 실시간 전송",
        description:
          "fleetmap은 배송 차량을 위한 모니터링 콘솔입니다. 각 차량의 휴대폰이 위치를 보내면 사무실 화면에 모든 차량의 움직임이 정류지, ETA, 지연과 함께 표시됩니다. 실시간 전파는 Supabase Realtime이 맡아서 Redis도, 직접 만든 WebSocket 서버도 없습니다. 경로는 스위스 지도 데이터로 직접 호스팅한 OSRM에서 가져옵니다. 벽걸이 터치스크린에서 실제 운영 중입니다.",
      },
    },
  },
  skills: {
